"""
Training Script V5 - ByT5-small Byte-Level Fine-tuning
========================================================
Fine-tunes google/byt5-small on multilingual Sourashtra/Tamil/English data.

Key innovations over V4:
  - ByT5-small: byte-level tokenization (no subword splitting)
  - Every character is a separate token → perfect for transliteration
  - Same Tamil cross-lingual data as V4 for fair comparison
...
from data_loader_v5 import load_and_prepare_data
...
def main():
    ...
    # ByT5: simpler generation config (byte-level doesn't need n-gram blocking)
    ...
    sample_text = "translate Sourashtra to English: paal"
    ...
    # ── Train! ──
    print("\n" + "=" * 70)
    print("  STARTING TRAINING (V5 - ByT5-small + Tamil)")
    print("  Model:   ByT5-small (300M params, byte-level)")
    print("  Tasks:   Sourashtra→English + Tamil→English")
    ...
    # Save best model
    best_dir = os.path.join(config.CHECKPOINT_DIR, "best_model_v5")
    ...
    print("\n" + "=" * 70)
    print("  FINAL EVALUATION ON TEST SET (Sourashtra → English only)")
    print("=" * 70)
    ...
    print_all_metrics(full_metrics, "V5 TEST RESULTS (ByT5-small + Tamil)")

    # ── Save Results ──
    results = {
        "model": config.MODEL_NAME,
        "version": "V5",
        "approach": "ByT5-small byte-level + Tamil cross-lingual transfer",
    ...
    with open(os.path.join(config.RESULTS_DIR, "test_results_v5.json"), "w") as f:
        json.dump(results, f, indent=2)

    with open(os.path.join(config.RESULTS_DIR, "test_predictions_v5.json"), "w",
              encoding="utf-8") as f:
    ...
    print(f"{'='*70}")
    v5_em = full_metrics["exact_match_accuracy"]
    print(f"  V1 (Char GRU):           0.42%")
    print(f"  V2 (Transformer+BPE):    2.56%")
    print(f"  V3 Neural (T5-small):    6.01%")
    print(f"  V3 Hybrid:               7.47%")
    print(f"  V4 Neural (T5+Tamil):    5.80%")
    print(f"  V4 Hybrid:               7.68%")
    print(f"  V5 Neural (ByT5+Tamil):  {v5_em:.2f}%")
    ...
    print(f"  Next: run evaluate_v5.py for enhanced hybrid retrieval")
